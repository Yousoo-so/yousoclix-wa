...
Baileys and its maintainers cannot be held liable for misuse of this application, as stated in the [MIT license](https://github.com/Yousoo-so/yousoclix-wa/blob/main/LICENSE).
...
```
yarn add @Yousoo-so/yousoclix-wa
```

Use the edge version (no guarantee of stability, but latest fixes + features)
```
yarn add github:Yousoo-so/yousoclix-wa
```

Then import your code using:
``` ts 
import makeWASocket from '@Yousoo-so/yousoclix-wa'
```
...
``` ts
import makeWASocket, { DisconnectReason } from '@Yousoo-so/yousoclix-wa'
...
``` ts
import makeWASocket, { BufferJSON, useMultiFileAuthState } from '@Yousoo-so/yousoclix-wa'
...
``` ts
import makeWASocket, { makeInMemoryStore } from '@Yousoo-so/yousoclix-wa'
...
``` ts
import { MessageType, MessageOptions, Mimetype } from '@Yousoo-so/yousoclix-wa'
...
- **MiscGenerationOptions**: some extra info about the message. It can have the following __optional__ values:
    ...
    const info: MessageOptions = {
        ...
        mimetype: Mimetype.pdf, /* (for media messages) specify the type of media (optional for all media types except documents),
                                    import {Mimetype} from '@Yousoo-so/yousoclix-wa'
                                */
...
import { downloadMediaMessage } from '@Yousoo-so/yousoclix-wa'
